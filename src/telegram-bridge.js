...
export class TelegramBridge extends EventEmitter {
    ...
    async killOrphanedProcesses() {
        ...
        try {
            ...
            const result = execSync(`pgrep -f "telegram-text-bridge.py.*${this.config.sessionPath}" 2>/dev/null || true`, { encoding: "utf-8" }).trim();
            if (result) {
    ...
    async request(action, payload) {
        ...
        const request = { id, action: action, payload };
...
//# sourceMappingURL=telegram-bridge.js.map