...
const pendingVoiceContexts = new Map<string, VoiceContext>();
 ...
 */
export function storeVoiceContext(ctx: VoiceContext): void {
 ...
 */
export function consumeVoiceContext(userId: string): VoiceContext | null {
  // Find the most recent voice context for this user
  let latestContext: VoiceContext | null = null;
 ...
 */
export function hasVoiceContext(userId: string): boolean {
 ...
 */
export function peekVoiceContext(userId: string): VoiceContext | null {
  for (const ctx of pendingVoiceContexts.values()) {
    if (ctx.userId === userId && ctx.wantsVoiceResponse) {
      return ctx;
    }
 ...
 */
export function getPendingVoiceContextCount(): number {
