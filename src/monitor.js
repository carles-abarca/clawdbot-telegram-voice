...
import { storeVoiceContext, consumeVoiceContext } from "./voice-context.js";
// Queue functions will be loaded dynamically at runtime
let enqueueFollowupRun = null;
let scheduleFollowupDrain = null;
let queueFunctionsLoaded = false;
async function loadQueueFunctions() {
    if (queueFunctionsLoaded)
        return;
    try {
...
export async function monitorTelegramUserbot(opts) {
    ...
    async function handleIncomingMessage(data) {
        ...
        if (!sentMessage) {
            ...
            if (enqueueFollowupRun && scheduleFollowupDrain) {
                ...
                core.system.enqueueSystemEvent(`[PENDING] Telegram${isVoiceMessage ? " voice" : ""} from ${senderName}: ${preview}`, {
                    sessionKey: route.sessionKey,
        ...
        if (sentMessage) {
            ...
            core.system.enqueueSystemEvent(`Telegram Userbot message from ${senderName}: ${preview}`, {
                sessionKey: route.sessionKey,
