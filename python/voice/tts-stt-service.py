...
# Directori base
BASE_DIR = Path.home() / ".clawdbot" / "tts-stt-service"
CONFIG_PATH = BASE_DIR / "config.json"
STATE_PATH = BASE_DIR / "conversation-state.json"
SESSION_PATH = BASE_DIR / "session"
CALL_SESSION_PATH = BASE_DIR / "session-call"  # Separate session for calls
TMP_DIR = Path(tempfile.gettempdir()) / "tts-stt"
...
class CallService:
    ...
    async def start(self):
        ...
        # Check if session exists (use separate session for calls)
        session_file = CALL_SESSION_PATH.with_suffix('.session')
        if not session_file.exists():
            # Copy from main session if it doesn't exist
            main_session = SESSION_PATH.with_suffix('.session')
            if main_session.exists():
                import shutil
                shutil.copy(main_session, session_file)
                log.info(f"Created call session from main session: {session_file}")
            else:
...
class VoiceService:
    ...
    async def transcribe(self, audio_path: str, user_id: Optional[str] = None, force_language: Optional[str] = None) -> Dict:
        """Transcriu àudio amb auto-detecció intel·ligent d'idioma"""
        ...
        # Obtenir idioma de l'usuari com a fallback
        user_language = None
        if user_id:
            user_language = self.state.get_language(str(user_id))
        fallback_language = user_language or self.default_language
        
        # Si tenim idioma forçat, usar-lo directament
        if force_language:
            language = force_language
            confidence = 1.0
            log.info(f"  Forced language: {language}")
        else:
            # Auto-detectar idioma amb probabilitat
            detected_lang, confidence = await self._detect_language_with_confidence(wav_path)
            
            CONFIDENCE_THRESHOLD = 0.50  # 50%
            
            if confidence >= CONFIDENCE_THRESHOLD:
                # Alta confiança: usar idioma detectat
                language = detected_lang
                log.info(f"  Auto-detected: {detected_lang} (p={confidence:.2%}) ✓")
                # Actualitzar preferència de l'usuari
                if user_id and detected_lang in self.SUPPORTED_LANGUAGES:
                    self.state.set_language(str(user_id), detected_lang)
            else:
                # Baixa confiança: usar idioma de l'usuari/default
                language = fallback_language
                log.info(f"  Auto-detected: {detected_lang} (p={confidence:.2%}) → using fallback: {language}")
        
        # Executar Whisper amb l'idioma determinat
        ...
        log.info(f"  Transcribing with language: {lang_code}")
        ...
        try:
            ...
            # Retornar l'idioma usat (no auto-detectat)
            return {
                "text": text,
                "language": language,  # L'idioma que hem usat
    ...
    async def _detect_language_with_confidence(self, wav_path: str) -> tuple:
        """Detecta idioma amb Whisper i retorna (idioma, probabilitat)"""
        import re
        ...
        try:
            ...
            # Parsejar "auto-detected language: XX (p = 0.XXXXX)"
            output = stderr.decode() + stdout.decode()
            match = re.search(r'auto-detected language: (\w+) \(p = ([\d.]+)\)', output)
            
            if match:
                detected_lang = match.group(1).lower()
                probability = float(match.group(2))
                
                # Mapejar codis de Whisper als nostres
                lang_map = {
                    "ca": "ca", "catalan": "ca",
                    "es": "es", "spanish": "es",
                    "en": "en", "english": "en",
                    "ko": "ko", "korean": "ko",  # Per debug
                }
                language = lang_map.get(detected_lang, detected_lang)
                
                return (language, probability)
            
            # Si no podem parsejar, retornem default amb baixa confiança
...
def load_config() -> Dict:
    ...
    if clawdbot_config.exists():
        ...
        userbot_config = config.get("channels", {}).get("tts-stt-service", {})
        return {
...
async def main():
    ...
    # NOTA: Les trucades es gestionen via telegram-text-bridge.py (no aquí)
    # El voice-service només fa TTS/STT per evitar conflictes de sessió Pyrogram
    call_service = None
    log.info("📞 Call service DISABLED (trucades via bridge)")
